...
fn spawn_audio_thread(sink: tokio::sync::mpsc::Sender<Vec<i16>>){
...
async fn init_webrtc_audio_destination(mut audio_buf_rx: tokio::sync::mpsc::Receiver<Vec<i16>>) -> Result<(), anyhow::Error> {
...
async fn main() -> Result<()> {
    ...
    spawn_audio_thread(audio_buf_tx);
    init_webrtc_audio_destination(audio_buf_rx).await;
    Ok(())
}